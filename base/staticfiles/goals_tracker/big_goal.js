...
Array.from(allDailySystemBox).forEach((dailySystem) => {
  dailySystem.addEventListener("click", () => {
    ...
    console.log(date, bigGoal, actionText);
...
Focus on today first.`);
       ...
       // Check if completedSystems exists and has the property for the given date
        if (completedSystems && completedSystems.hasOwnProperty(date)) {
          // Ensure bigGoal exists in completedSystems[date] and is an array before checking for actionText
          const bigGoalActions = completedSystems[date][bigGoal] || [];
          
          if (Array.isArray(bigGoalActions) && bigGoalActions.includes(actionText)) {
            // Action is already marked as completed, so uncheck it
            ...
            // Action is not completed, so mark it as complete
          ...
          // If completedSystems or completedSystems[date] does not exist, treat it as not completed and mark as complete
          checkBox.classList.add("daily-system-box--check");
          dailyAction.classList.add("daily-system-box--check");
